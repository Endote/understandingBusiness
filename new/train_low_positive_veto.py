...
def labels(frame: pd.DataFrame, mode: str) -> np.ndarray:
    sales = frame["sales_target"].to_numpy(dtype=np.float32)
    if mode == "le1":
        return (sales <= 1.0).astype(np.float32)
    if mode == "eq1":
        return (sales == 1.0).astype(np.float32)
    if mode == "positive_le1":
        return ((sales > 0.0) & (sales <= 1.0)).astype(np.float32)
    raise ValueError(f"Unsupported label mode: {mode}")


def metrics(frame: pd.DataFrame, probability: np.ndarray, split: str, candidate_threshold: float, label_mode: str) -> dict[str, object]:
    y = labels(frame, label_mode)
    ...
    result = {
        ...
        "candidate_label_rate": float(y[candidate].mean()) if candidate.any() else math.nan,
    ...
    for top_frac in (0.10, 0.20, 0.30, 0.40, 0.50):
        ...
        result[f"label_recall_at_top_{int(top_frac * 100)}"] = float(y[idx].sum() / max(y.sum(), 1.0))
        result[f"label_rate_at_top_{int(top_frac * 100)}"] = float(y[idx].mean())
    return result


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a low-positive veto model for SIP/Weeklies predicted-positive rows.")
    ...
    parser.add_argument("--label-mode", choices=["le1", "eq1", "positive_le1"], default="le1")
...
def main() -> int:
    ...
    log(
        "Candidate low-veto rows: "
        f"train={len(candidate_train)}, valid={len(candidate_valid)}, "
        f"train label rate={labels(candidate_train, args.label_mode).mean():.4f}"
    )
    ...
    y_train = labels(candidate_train, args.label_mode)
    y_valid = labels(candidate_valid, args.label_mode)
    positives = float(y_train.sum())
    negatives = float(len(y_train) - positives)
    scale_pos_weight = negatives / positives if positives > 0 else 1.0

    log("Encoding low-veto train and validation matrices")
    ...
    log(f"Training low-positive veto: scale_pos_weight={scale_pos_weight:.3f}")
    booster = xgb.train(
        params=params,
        dtrain=dtrain,
    ...
    for split, split_df in (("validation", valid_df), ("test", test_df)):
        log(f"Scoring low-veto probabilities for {split}")
        ...
        metric_rows.append(metrics(split_df, probability, split, args.candidate_threshold, args.label_mode))
        out = split_df[KEY_COLUMNS + ["split", "sales_target", "positive_sale_flag", "raw_probability"]].copy()
        out["low_positive_veto_probability"] = probability
        out.to_csv(run_dir / f"{split}_low_positive_veto_predictions.csv", index=False)
        del encoded, dmatrix
        gc.collect()

    metric_frame = pd.DataFrame.from_records(metric_rows)
    metric_frame.to_csv(run_dir / "low_positive_veto_metrics.csv", index=False)
    summary = {
    ...
    log(f"Wrote low-positive veto outputs to {run_dir}")
    return 0
