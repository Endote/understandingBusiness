    ...
    for alpha in alphas:
        for band_alpha in band_alphas:
            for scale in scales:
                ...
                row["scale"] = scale
                rows.append(row)
    sort_col = "strict_composite_decile_score" if selection_metric == "strict" else "composite_decile_score"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--selection-metric", choices=["composite", "strict"], default="composite")
...
def main() -> int:
    ...
    run_dir = (
        ...
        / args.family.lower()
        / args.stage
        / f"tail_q{int(args.tail_quantile * 100)}"
    ...
    train_tail_labels = tail_labels(train_df, args.tail_quantile)
    valid_tail_labels = tail_labels(valid_df, args.tail_quantile)
    booster, train_prob, valid_prob = train_classifier(
        ...
        label=f"tail_q{int(args.tail_quantile * 100)}",
        rounds=args.rounds,
    ...
    if args.enable_band_layer:
        if args.band_lower_quantile >= args.band_upper_quantile:
            raise ValueError("--band-lower-quantile must be below --band-upper-quantile")
        train_band_labels = band_labels(train_df, args.band_lower_quantile, args.band_upper_quantile)
        valid_band_labels = band_labels(valid_df, args.band_lower_quantile, args.band_upper_quantile)
        band_booster, train_band_prob, valid_band_prob = train_classifier(
            ...
            label=f"band_q{int(args.band_lower_quantile * 100)}_q{int(args.band_upper_quantile * 100)}",
            rounds=args.rounds,
            early_stopping=args.early_stopping,
            seed=args.seed + 17,
        )
    ...
    log(
        ...
        f"alpha={alpha:.3f}, band_alpha={band_alpha:.3f}, scale={scale:.3f}, "
    ...
    classifier_rows = [
        classifier_metrics(train_df, train_prob, train_tail_labels, f"tail_q{int(args.tail_quantile * 100)}", "train"),
        classifier_metrics(valid_df, valid_prob, valid_tail_labels, f"tail_q{int(args.tail_quantile * 100)}", "validation"),
        classifier_metrics(test_df, test_prob, tail_labels(test_df, args.tail_quantile), f"tail_q{int(args.tail_quantile * 100)}", "test"),
    ]
    if args.enable_band_layer:
        classifier_rows.extend(
            [
                classifier_metrics(
                    train_df,
                    train_band_prob,
                    band_labels(train_df, args.band_lower_quantile, args.band_upper_quantile),
                    f"band_q{int(args.band_lower_quantile * 100)}_q{int(args.band_upper_quantile * 100)}",
                    "train",
                ),
                classifier_metrics(
                    valid_df,
                    valid_band_prob,
                    band_labels(valid_df, args.band_lower_quantile, args.band_upper_quantile),
                    f"band_q{int(args.band_lower_quantile * 100)}_q{int(args.band_upper_quantile * 100)}",
                    "validation",
                ),
                classifier_metrics(
                    test_df,
                    test_band_prob,
                    band_labels(test_df, args.band_lower_quantile, args.band_upper_quantile),
                    f"band_q{int(args.band_lower_quantile * 100)}_q{int(args.band_upper_quantile * 100)}",
                    "test",
                ),
            ]
